# ============================================================================
# 4_metrics-spei.R — Compute SPEI for stations with PET
#
# For each SPEI-eligible station, computes water balance (PRCP - PET),
# then fits GLO distribution to get SPEI at all timescales.
# ============================================================================

source(file.path(Sys.getenv("PROJECT_DIR",
  file.path(Sys.getenv("HOME"), "mco-GHCNd")), "R", "pipeline-common.R"))

env   = setup_pipeline_env()
paths = setup_pipeline_paths(env)
source_drought_functions(env$PROJECT_DIR)

msg("Step 4: Computing SPEI")

# ---- Load stations and parse config ------------------------------------------

spei_stations = readRDS(file.path(paths$station_lists, "stations_spei.rds"))
timescales    = parse_timescales(env$TIMESCALES)
clim_periods  = parse_clim_periods(env$CLIM_PERIODS)

msg(sprintf("Stations: %d | Timescales: %d | Clim periods: %d",
            nrow(spei_stations), length(timescales), length(clim_periods)))

# ---- Compute SPEI for each station -------------------------------------------

compute_spei_for_station = function(sid) {
  tryCatch({
    rds_path = file.path(paths$station_data, sprintf("%s.rds", sid))
    if (!file.exists(rds_path))
      return(list(station_id = sid, skip_reason = "no RDS file"))

    dt = readRDS(rds_path)
    if (!("prcp" %in% names(dt)))
      return(list(station_id = sid, skip_reason = "no PRCP column"))
    if (!("pet" %in% names(dt)))
      return(list(station_id = sid, skip_reason = "no PET column (Hargreaves-Samani not computed — missing TMAX/TMIN?)"))

    # Compute daily water balance
    dt[, wb := prcp - pet]

    valid_dates = dt[!is.na(wb), date]
    if (length(valid_dates) == 0)
      return(list(station_id = sid, skip_reason = "all water balance values are NA (PRCP or PET missing)"))
    ref_date = max(valid_dates)

    days_since = as.integer(Sys.Date() - ref_date)
    if (days_since > env$MAX_REPORTING_LATENCY)
      return(list(station_id = sid, skip_reason = sprintf(
        "last obs %s (%d days ago, max %d)", ref_date, days_since, env$MAX_REPORTING_LATENCY)))

    current_year = as.integer(format(ref_date, "%Y"))

    results = list()
    na_reasons = character(0)

    for (cp in clim_periods) {
      clim_years = get_clim_years(cp, current_year, env$START_YEAR)

      for (ts in timescales) {
        window = ts$get_days(ref_date)
        if (window < 1) next

        label = paste0("spei_", ts$label, "_", cp$label)

        wb_dt = dt[, .(date, value = wb)]

        # Current water balance (independent of clim_years)
        current_val = compute_current_value(
          wb_dt, ref_date, window,
          agg_fn = sum, min_obs_frac = env$MIN_OBS_FRACTION
        )

        if (is.na(current_val)) {
          results[[label]] = NA_real_
          na_reasons = c(na_reasons, sprintf(
            "%s: current %dd window has missing PRCP or PET", label, window))
          next
        }

        # Reference distribution from clim_years
        clim_vec = extract_clim_vector(
          wb_dt, ref_date, window, clim_years,
          agg_fn = sum, min_obs_frac = env$MIN_OBS_FRACTION
        )

        clim_vec_clean = clim_vec[!is.na(clim_vec)]
        n_avail = length(clim_vec_clean)
        n_total = length(clim_vec)

        if (n_avail < 3) {
          results[[label]] = NA_real_
          na_reasons = c(na_reasons, sprintf(
            "%s: only %d/%d valid clim years (missing PRCP or PET in %dd windows)",
            label, n_avail, n_total, window))
          next
        }

        spei_val = glo_fit_spei(clim_vec_clean, current_val,
                                export_opts = "SPEI",
                                climatology_length = length(clim_vec_clean))

        results[[label]] = spei_val
      }
    }

    results$station_id = sid
    results$last_obs_date = as.character(ref_date)
    results$skip_reason = NA_character_
    if (length(na_reasons) > 0) results$na_details = paste(na_reasons, collapse = "; ")
    return(results)

  }, error = function(e) {
    return(list(station_id = sid, skip_reason = sprintf("error: %s", conditionMessage(e))))
  })
}

spei_results = run_parallel(
  spei_stations$id,
  compute_spei_for_station,
  cores = env$CORES,
  description = "stations (SPEI)"
)

# ---- Collect results and log diagnostics -------------------------------------

spei_results = spei_results[!sapply(spei_results, is.null)]

skipped = spei_results[sapply(spei_results, function(r) {
  !is.null(r$skip_reason) && !is.na(r$skip_reason)
})]
computed = spei_results[sapply(spei_results, function(r) {
  is.null(r$skip_reason) || is.na(r$skip_reason)
})]

if (length(skipped) > 0) {
  msg(sprintf("SPEI skipped for %d stations:", length(skipped)))
  skip_reasons = sapply(skipped, function(r) r$skip_reason)
  reason_table = table(skip_reasons)
  for (reason in names(reason_table)) {
    msg(sprintf("  %d stations: %s", reason_table[[reason]], reason))
  }
}

if (length(computed) > 0) {
  spei_dt = rbindlist(lapply(computed, function(r) {
    r$skip_reason = NULL
    r$na_details = NULL
    as.data.table(r)
  }), fill = TRUE)
  saveRDS(spei_dt, file.path(paths$derived_dir, "spei_results.rds"))

  na_stations = computed[sapply(computed, function(r) !is.null(r$na_details))]
  if (length(na_stations) > 0) {
    msg(sprintf("SPEI partial results (%d stations had some NA timescales):", length(na_stations)))
    for (r in na_stations[1:min(5, length(na_stations))]) {
      msg(sprintf("  %s: %s", r$station_id, r$na_details))
    }
    if (length(na_stations) > 5) msg(sprintf("  ... and %d more", length(na_stations) - 5))
  }

  msg(sprintf("Step 4 complete: SPEI computed for %d / %d stations",
              nrow(spei_dt), nrow(spei_stations)))
} else {
  msg("WARNING: No SPEI results produced")
}
